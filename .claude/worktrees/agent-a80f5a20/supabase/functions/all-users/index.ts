...
function normalizeUsers(rows: any[], appName: string, usersTable: string): NormalizedUser[] {
  return rows.map((r) => {
    ...
    // Dealership info (Demolight joins dealerships table)
    const dealership = r.dealerships || r.dealership || null;
    const dealershipName = typeof dealership === 'object' && dealership
      ? (dealership.name || null)
      : (r.dealership_name || null);
    const subscriptionStatus = typeof dealership === 'object' && dealership
      ? (dealership.subscription_status || null)
    ...
    const fetches = (appConfigs || []).map(async (app: any) => {
      try {
        ...
        if (isLocal) {
          ...
          // Select all + join dealerships for subscription data
          const selectQuery = usersTable === 'users'
            ? `*,dealerships(name,subscription_status)`
            : '*';
          const res = await fetch(
            `${app.supabase_url}/rest/v1/${usersTable}?select=${encodeURIComponent(selectQuery)}&limit=200`,
            {
              headers: {
                apikey: svcKey,
