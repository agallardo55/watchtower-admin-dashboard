...
const activityTagStyle: Record<string, React.CSSProperties> = {
  deploy:  { color: 'var(--accent)', background: 'rgba(74,222,128,0.08)',  border: '1px solid rgba(74,222,128,0.15)' },
  feature: { color: 'var(--accent)', background: 'rgba(74,222,128,0.08)',  border: '1px solid rgba(74,222,128,0.15)' },
  signup:  { color: 'var(--info)', background: 'rgba(59,130,246,0.08)',  border: '1px solid rgba(59,130,246,0.15)' },
  spec:    { color: 'var(--info)', background: 'rgba(59,130,246,0.08)',  border: '1px solid rgba(59,130,246,0.15)' },
  schema:  { color: 'var(--purple)', background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.15)' },
  ticket:  { color: 'var(--warning)', background: 'rgba(245,158,11,0.08)',  border: '1px solid rgba(245,158,11,0.15)' },
  alert:   { color: 'var(--danger)', background: 'rgba(239,68,68,0.08)',   border: '1px solid rgba(239,68,68,0.15)' },
  launch:  { color: 'var(--danger)', background: 'rgba(239,68,68,0.08)',   border: '1px solid rgba(239,68,68,0.15)' },
};
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
          {[1,2,3,4].map(i => (
            <div key={i} className="terminal-card" style={{ padding: 20, height: 120, opacity: 0.5 }}>
              <div style={{ height: 10, background: 'var(--bg-elevated)', borderRadius: 2, width: '60%', marginBottom: 12 }} />
              <div style={{ height: 28, background: 'var(--bg-elevated)', borderRadius: 2, width: '40%', marginBottom: 8 }} />
              <div style={{ height: 8, background: 'var(--bg-elevated)', borderRadius: 2, width: '80%' }} />
            </div>
          ))}
        </div>
      </div>
  ...
  if (statsError) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '40vh', textAlign: 'center' }}>
        <div style={{ fontSize: 12, color: 'var(--danger)', marginBottom: 8 }}>// error: failed to load dashboard</div>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 16 }}>{statsError}</div>
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>// DASHBOARD</div>
        <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)' }}>Mission Control</div>
      </div>

      {/* KPI — code syntax card */}
      <div className="terminal-card-glow" style={{ marginBottom: 24, padding: '20px 24px' }}>
        <div style={{ color: 'var(--accent)', fontSize: 12, fontWeight: 600, marginBottom: 16 }}>const system = {'{'}</div>
        ...
        <div style={{ color: 'var(--accent)', fontSize: 12, fontWeight: 600, marginTop: 16 }}>{'}'}</div>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 16 }} className="lg:grid-cols-[1fr_380px] grid-cols-1">
          ...
          <div className="terminal-card" style={{ overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>## app_status</span>
              <button style={{ fontSize: 10, color: 'var(--accent)', background: 'transparent', border: 'none', cursor: 'pointer' }}>view_all →</button>
            </div>
                    ...
                    <tr key={app.name}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div>
                            <div style={{ color: 'var(--text-primary)', fontWeight: 500, fontSize: 12 }}>{app.name}</div>
                            <div style={{ color: 'var(--text-muted)', fontSize: 10 }}>{app.category || '—'}</div>
                          </div>
                        </div>
                      </td>
                      <td>
                      ...
                      <td>
                        <span style={{ color: 'var(--text-dim)', fontSize: 11, maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>{app.db || '—'}</span>
                      </td>
                      <td style={{ textAlign: 'right', color: 'var(--accent)', fontWeight: 600 }}>{app.users}</td>
                      <td>
                        <div style={{ display: 'flex', gap: 4 }}>
                          <button style={{ padding: '4px 6px', background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', borderRadius: 3 }}
                          ...
                          {app.url && (
                            <a href={app.url} target="_blank" style={{ padding: '4px 6px', background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', borderRadius: 3, display: 'inline-flex', alignItems: 'center' }}
          ...
          <div className="terminal-card" style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border-subtle)' }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>## activity_log</span>
            </div>
            <div style={{ padding: '12px 16px', flex: 1, overflow: 'hidden' }}>
              {recentActivity.length === 0 ? (
                <div style={{ color: 'var(--text-muted)', fontSize: 12, padding: '24px 0', textAlign: 'center' }}>// no recent activity</div>
              ) : (
                  ...
                  return (
                    <div key={idx} className="terminal-log-line">
                      ...
                      <span className="terminal-log-desc" style={{ fontSize: 11 }}>
                        <span style={{ color: 'var(--text-secondary)' }}>{activity.app.toLowerCase()} — </span>
                        {activity.action}
                      </span>
                    </div>
                  );
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="terminal-card" style={{ padding: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>## user_growth</div>
                <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>cumulative active users across apps</div>
              </div>
              <div style={{ display: 'flex', gap: 4 }}>
            ...
            {chartData.length === 0 ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 250 }}>
                <span style={{ color: 'var(--text-muted)', fontSize: 12 }}>// no analytics data yet</span>
              </div>
            ) : (
              <div style={{ width: '100%', height: 280 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: 'var(--chart-axis)', fontSize: 10, fontFamily: 'JetBrains Mono, monospace' }} dy={8} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fill: 'var(--chart-axis)', fontSize: 10, fontFamily: 'JetBrains Mono, monospace' }} />
                    <Tooltip
                      contentStyle={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border)', borderRadius: '4px', fontSize: '11px', fontFamily: 'JetBrains Mono, monospace' }}
                      itemStyle={{ color: 'var(--text-primary)' }}
                      labelStyle={{ color: 'var(--text-secondary)', marginBottom: 4 }}
                    />
                    <Area type="monotone" dataKey="buybid" stroke="var(--accent)" strokeWidth={2} fillOpacity={1} fill="url(#colorBuybid)" name="BuybidHQ" />
            ...
            {liveApps.map((app) => (
              <div key={app.name} className="terminal-card" style={{ padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>{app.name}</div>
                    <span className={getStatusBadgeClass(app.status)}>{app.status}</span>
                  </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                  <div>
                    <div style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 4 }}>USERS</div>
                    <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--accent)' }}>{app.users}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 4 }}>TABLES</div>
                    <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)' }}>{app.tableCount || 0}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-muted)', marginBottom: 4 }}>SHARE</div>
                    <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)' }}>{totalUsers > 0 ? Math.round((app.users / totalUsers) * 100) : 0}%</div>
                  </div>
                </div>
              </div>
            ))}
  ...
  return (
    <div>
      <div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 4 }}>
        ...
        <span style={{ color: 'var(--purple)' }}>{label}</span>
        <span style={{ color: 'var(--text-secondary)' }}>:</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <span style={{ fontSize: 28, fontWeight: 700, color: warn ? 'var(--warning)' : 'var(--accent)', lineHeight: 1 }}>{value}</span>
