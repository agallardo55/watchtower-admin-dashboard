...
export default function SchemaBrowser() {
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex gap-6 max-w-7xl mx-auto overflow-hidden">
      {/* Table List */}
      <div className="w-[300px] flex flex-col glass rounded-none overflow-hidden">
        <div className="p-4 border-b border-[#1e293b] bg-white/2">
              ...
              <div className="space-y-0.5">
                {group.tables.map((table) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 rounded-sm text-xs transition-all flex items-center gap-2 ${
                      selectedTable === table
                        ? 'bg-blue-600 text-white font-semibold'
                        : 'hover:bg-[#111] text-slate-400 hover:text-slate-200'
                    }`}
                  >
      ...
      {/* Table Detail */}
      <div className="flex-1 flex flex-col glass rounded-none overflow-hidden">
        <div className="p-6 border-b border-[#1e293b] flex items-center justify-between bg-white/2">
          <div>
          ...
          <div className="flex gap-2">
            <button title="Coming soon" className="px-3 py-1.5 bg-[#111] hover:bg-[#1e293b] rounded-sm text-xs font-semibold border border-slate-700 transition-colors">Export SQL</button>
            <button title="Coming soon" className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-sm text-xs font-semibold transition-colors">New Migration</button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <div className="p-6 space-y-8">
            ...
            <div className="space-y-4">
              ...
              {columnsError && (
                <div className="rounded-none border border-red-500/20 bg-red-500/5 p-4 text-xs text-red-400">
                  {columnsError}
                </div>
              )}

              {!columnsLoading && !columnsError && !selectedTable && (
                <div className="rounded-none border border-[#1e293b] p-6 text-center text-xs text-slate-500">
              ...
              {!columnsLoading && !columnsError && columns.length > 0 && (
                <div className="rounded-none border border-[#1e293b] overflow-hidden">
                  <table className="w-full text-left text-xs font-mono">
                    <thead className="bg-[#111] text-slate-500 border-b border-[#1e293b]">
                      <tr>
                    ...
                    <tbody className="divide-y divide-[#1e293b] bg-[#0a0a0a]">
                      {columns.map((col) => (
                        <tr key={col.column_name} className="hover:bg-[#111]">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-none border border-[#1e293b] overflow-hidden bg-[#111] p-6 flex flex-col items-center justify-center h-48 border-dashed">
