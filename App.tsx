...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    <>
      <div className={`p-4 lg:p-6 flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
        <div className={`flex items-center gap-1.5 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
          ...
          {!isSidebarCollapsed && <span className="font-bold text-lg text-white tracking-tight">Watchtower</span>}
        </div>
        <button
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2 text-xs transition-all group relative ${
                    ...
                    isParentActive
                      ...
                      : 'hover:bg-[#111] text-slate-500 hover:text-slate-300 border-l-2 border-transparent'
                  }`}
                >
                      ...
                      return (
                        ...
                        >
                          {child.iconUrl ? (
                            ...
                            <span className="text-slate-600">{React.createElement((icons as Record<string, React.FC<IconProps>>)[child.icon])}</span>
                          ) : child.icon ? (
                            <span className="text-slate-600"><icons.package className="w-4 h-4" /></span>
                          ) : null}
      ...
      {/* Collapse toggle */}
      <div className="hidden lg:flex justify-center p-4 border-t border-[#1e293b]">
        <button
          onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
          className="flex items-center justify-center p-1.5 hover:bg-[#111] text-slate-600 hover:text-slate-400 transition-colors"
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#0a0a0a] text-slate-100">
      ...
      {/* Sidebar */}
      <aside style={isSidebarCollapsed ? { width: 80, minWidth: 80, maxWidth: 80 } : { width: 240, minWidth: 240, maxWidth: 240 }} className="flex flex-col border-r transition-all duration-300 overflow-hidden flex-shrink-0 bg-[#0a0a0a] border-[#1e293b] hidden lg:flex">
        {sidebarContent}
      </aside>

      {/* Mobile sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-[280px] flex flex-col border-r transition-transform duration-300 ease-in-out lg:hidden bg-[#0a0a0a] border-[#1e293b] ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {sidebarContent}
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Mobile top bar */}
        <div className="sticky top-0 z-30 flex items-center justify-between px-3 py-2.5 border-b lg:hidden bg-[#0a0a0a] border-[#1e293b]">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="p-2 hover:bg-[#111] text-slate-400 transition-colors"
            >
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-base text-white">Watchtower</span>
            </div>
          </div>
        </div>

        {/* Desktop Header */}
        <header className="sticky top-0 z-30 h-14 hidden lg:flex items-center justify-between px-6 border-b bg-[#0a0a0a] border-[#1e293b]">
          ...
          <div className="flex items-center gap-4">
            <div className="relative group">
              ...
              <input
                ...
                className="pl-10 pr-4 py-1.5 text-xs w-56 border bg-[#111] border-[#1e293b] focus:outline-none focus:border-blue-500 transition-colors"
              />
            </div>
            ...
            <div className="flex items-center gap-3 ml-2">
              <div className="text-right">
                <p className="text-xs font-medium text-slate-200">Adam Gallardo</p>
                <p className="text-[9px] text-slate-600 uppercase font-bold tracking-wider">Super Admin</p>
              </div>
              <div className="w-8 h-8 bg-[#111] border border-slate-600 flex items-center justify-center text-[10px] font-bold text-slate-300">AG</div>
            </div>
